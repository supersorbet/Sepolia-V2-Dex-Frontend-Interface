...
import { bscProvider, ethProvider, goerliProvider, goerliArbiProvider } from './provider'
...
const cakeBusdPairMap = { 
  [ChainId.ETHEREUM]: {
    address: Pair.getAddress(CAKE[ChainId.ETHEREUM], USDC[ChainId.ETHEREUM]),
    tokenA: CAKE[ChainId.ETHEREUM],
    tokenB: USDC[ChainId.ETHEREUM],
  },
  [ChainId.GOERLI]: {
    address: Pair.getAddress(CAKE[ChainId.GOERLI], USDC[ChainId.GOERLI]),
    tokenA: CAKE[ChainId.GOERLI],
    tokenB: USDC[ChainId.GOERLI],
  },
}

const getCakePrice = async (isTestnet: boolean) => {
  const pairConfig = cakeBusdPairMap[isTestnet ? ChainId.GOERLI : ChainId.ETHEREUM]
  const pairContract = new Contract(pairConfig.address, pairAbi, isTestnet ? goerliProvider : ethProvider)
  const reserves = await pairContract.getReserves()
  const { reserve0, reserve1 } = reserves
  const { tokenA, tokenB } = pairConfig
